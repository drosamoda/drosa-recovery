...
export const crmReadService = {
  ...
  async checkouts(query: Record<string, unknown>) {
    ...
    const [evaluated, messages] = await Promise.all([evaluateAbandonedCheckoutEligibilityBatch(rows).catch(() => rows.map(() => null)), prisma.messageLog.findMany({ where: { entityType: 'abandoned_checkout', entityId: { in: rows.map(row => row.id) } }, orderBy: { createdAt: 'desc' } })])
